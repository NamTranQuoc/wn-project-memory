...
- **API Retry Mechanism:** Mọi lệnh gọi embedding ra ngoài (qua LiteLLM) BẮT BUỘC phải có cơ chế Retry (đã có sẵn qua `tenacity` trong `litellm_client.py`) để chống rớt mạng hoặc Rate Limit từ nhà cung cấp. Không có tác vụ LLM chạy ngầm nào khác — việc trích xuất fact/task/watched_ref từ raw event là do agent tự làm trực tiếp (agent-driven writes), không qua distillation LLM tự động.
